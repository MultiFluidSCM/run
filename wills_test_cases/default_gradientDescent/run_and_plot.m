...
variables{end+1} = gradient_variable("mix entrain bt",       "btMixE", 0., 2.0, btMixE, -0.1);
variables{end+1} = gradient_variable("mix entrain bq",       "bqMixE", 0., 1.0, bqMixE, -0.1);
variables{end+1} = gradient_variable("mix detrain bw",       "bwMixD", 0., 1.0, bwMixD, -0.1);
variables{end+1} = gradient_variable("mix detrain bt",       "btMixD", 0., 2.0, btMixD, -0.1);
variables{end+1} = gradient_variable("mix detrain bq",       "bqMixD", 0., 1.0, bqMixD, -0.1);
variables{end+1} = gradient_variable("mix cloud entrain bw", "bwMicE", 0., 1.0, bwMicE, -0.1);
variables{end+1} = gradient_variable("mix cloud entrain bt", "btMicE", 0., 2.0, btMicE, -0.1);
variables{end+1} = gradient_variable("mix cloud entrain bq", "bqMicE", 0., 1.0, bqMicE, -0.1);
variables{end+1} = gradient_variable("mix cloud detrain bw", "bwMicD", 0., 1.0, bwMicD, -0.1);
variables{end+1} = gradient_variable("mix cloud detrain bt", "btMicD", 0., 2.0, btMicD, -0.1);
variables{end+1} = gradient_variable("mix cloud detrain bq", "bqMicD", 0., 1.0, bqMicD, -0.1);
variables{end+1} = gradient_variable("instab entrain bw",    "bwInsE", 0., 1.0, bwInsE, -0.1);
variables{end+1} = gradient_variable("instab entrain bt",    "btInsE", 0., 2.0, btInsE, -0.1);
variables{end+1} = gradient_variable("instab entrain bq",    "bqInsE", 0., 1.0, bqInsE, -0.1);
% variables{end+1} = gradient_variable("MYNN A1",              "mynnA1", 0., 1.0, mynnA1, -0.1);
% variables{end+1} = gradient_variable("MYNN A2",              "mynnA2", 0., 1.0, mynnA2, -0.1);
% variables{end+1} = gradient_variable("MYNN B1",              "mynnB1", 0., 50.0, mynnB1, -0.1);
% variables{end+1} = gradient_variable("MYNN B2",              "mynnB2", 0., 10.0, mynnB2, -0.1);
