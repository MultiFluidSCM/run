...
bw_params = [0.5,0.6,0.9,1.0];
bt_params = [0.5,0.6,0.9,1.0];
bq_params = [0.5,0.6,0.9,1.0];
bu_params = [1.0];
simulations = {};
rmses = [];
    ...
    for i_bt=1:length(bt_params)
        for i_bq=1:length(bq_params)
			for i_bu=1:length(bu_params)
				...
				id_bq = strrep(num2str(bq), '.', 'p');
				id_bu = strrep(num2str(bu), '.', 'p');
				
				id = join(['bw_',id_bw,'_bt_',id_bt,'_bq_',id_bq,'_bu_',id_bu]);
				...
				settings.model.constants.param.mix.bentrainq = bq;
				settings.model.constants.param.mix.bdetrainq = bq;
