function gradient_descent_global(settings, variables, iterations)
...
for i=1:iterations+1
    ...
    for v=1:length(variables)
        settings = update_all_settings(settings, folders, [variables(v)], [variables(v).value]);
    end
    ...
    for v=1:length(variables)
        ...
        variable_change = stochastic*min(max(-variable.gradient*variable.value, -0.1), 0.1);
        ...
        variables(v).gradient = 1000*(rmse_init-rmse) / (variables(v).value - value_new);
        ...
        if rmse < rmse_init
            ...
            variables(v).value = variables(v).value + (variables(v).value-value_new);
        end
